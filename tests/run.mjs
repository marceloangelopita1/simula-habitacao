...
test('Selected extra insurance increases CESH and ends before settlement',()=>{const f=fixtures[0],o={...f.input,system:'SAC'};const a=buildSchedule(o),b=buildSchedule({...o,extraPremiumMonthly:33});assert.ok(b.cesh>a.cesh);assert.equal(cents(b.rows[0].total-a.rows[0].total),33);assert.equal(b.rows.at(-1).extraPremium,0);assert.equal(b.initialInsurance,a.initialInsurance);return b.cesh;});
